...
static int parse_opt(option_t *opt, char *arg){
	int res = 1;
	char *short_rep_prefix = format_str("%s=", opt->short_rep);
	char *long_rep_prefix = format_str("%s=", opt->long_rep);
	if(!prefix(short_rep_prefix, arg))
		opt->eq = strlen(short_rep_prefix) - 1;
	else if(!prefix(long_rep_prefix, arg))
		opt->eq = strlen(long_rep_prefix) - 1;
	else
		goto cleanup;

	res = opt->callback(arg, opt);
cleanup:
	free(short_rep_prefix);
	free(long_rep_prefix);
	return res;
}
